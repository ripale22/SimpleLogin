...
public class MessageManager {
    ...
    public MessageManager(SimpleLoginPlugin plugin) {
        this.plugin = plugin;
        loadMessages();
    }
    ...
    public void loadMessages() {
        ...
        messagesFile = new File(messagesDir, language + ".yml");

        if (!messagesFile.exists()) {
            boolean copied = false;
            try (InputStream in = plugin.getResource("messages/" + language + ".yml")) {
                if (in != null) {
                    Files.copy(in, messagesFile.toPath());
                    copied = true;
                }
            } catch (IOException e) {
                plugin.getLogger().severe("No se pudo copiar messages/" + language + ".yml: " + e.getMessage());
            }

            if (!copied) {
                try (InputStream fallback = plugin.getResource("messages/es.yml")) {
                    if (fallback != null) {
                    ...
                    plugin.getLogger().severe("No se pudo crear el archivo de mensajes: " + e.getMessage());
                }
            }
        }
